    ...
    *Chat Completions* endpoint with Pydantic schema parsing enabled.
    ...
    try:
        ...
        if "quota" in error_msg.lower() or "rate limit" in error_msg.lower() or "429" in error_msg:
            print(f"🚨 API quota may be exhausted! Error details: {error_msg}")
        elif "401" in error_msg or "unauthorized" in error_msg.lower():
            print(f"🔑 API key authentication failed! Error details: {error_msg}")
        elif "403" in error_msg or "forbidden" in error_msg.lower():
            print(f"🚫 API access denied! Error details: {error_msg}")
...
async def _call_llm_unstructured(
    ...
    *Chat Completions* endpoint without any schema parsing. Returns raw text
    ...
    try:
        ...
        raise e  # Re-raise exception for upper-level handling
...
async def process_single_example_unstructured(
    ...
    *,
    ...
    try:
        ...
        print(f"📄 Failed example: {example.get('paper_id', 'unknown')} - {example.get('weakness_point', {}).get('content', 'no content')[:100]}...")
        return None
    except Exception as e:
        # 捕获所有其他异常
        error_type = type(e).__name__
